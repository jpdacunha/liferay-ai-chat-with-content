...
public class ChatService {
	...
	public ChatResponse doRag(Jwt jwt, ChatRequest chatRequest) throws IOException {
		...
		_log.info("chatResponse:" + response);
	...
	public Flux<ChatResponse> doStreamingRag(Jwt jwt, ChatRequest chatRequest) throws IOException {
		...
		streamingAssistant.chat(conversationId, userMessage.singleText())
								...
								.doOnError(error -> {
									_log.error("Streaming chat response error", error);
									chatResponseSink.tryEmitError(error);
								})
								.subscribe();

		Flux<ChatResponse> response = chatResponseSink.asFlux();
		if (_log.isInfoEnabled()) {
			_log.info("Streaming chat response initialized");
		}
	...
	private Map<String, List<String>> retrieveContent(Jwt jwt, ChatRequest chatRequest) {
		...
		if (_log.isInfoEnabled()) {
			boolean noResults = itemDatas == null || itemDatas.values().stream().allMatch(List::isEmpty);
			_log.info("itemDatas: " + (noResults ? "no results" : itemDatas));
		}
